...
import { Plug, KeyRound, Sparkles, Check, ArrowRight, ArrowLeft } from "lucide-react";
...
type Step = {
  ...
  ctaLabel: string;
  ctaPath: string;
  doneWhen: (ctx: { hasEndpoint: boolean; hasKey: boolean }) => boolean;
};
...
export function OnboardingWalkthrough() {
  ...
  const isDone = current.doneWhen({ hasEndpoint, hasKey });
    ...
    <Dialog open={open} onOpenChange={(v) => (v ? setOpen(true) : dismiss())}>
      <DialogContent className="max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Three short steps to put your first guarded request through the proxy.
          </DialogDescription>
        </DialogHeader>

        {/* Step pips — clickable so users can jump around */}
        <div className="flex items-center gap-2 pt-1">
          {STEPS.map((s, i) => {
            const done = s.doneWhen({ hasEndpoint, hasKey });
            const active = i === step;
            return (
              <button
                key={i}
        ...
        <DialogFooter className="flex-row justify-between sm:justify-between gap-2 pt-2">
          ...
          <div className="flex gap-2">
            ...
            <Button size="sm" onClick={() => goToStep(current)}>
              {current.ctaLabel}
